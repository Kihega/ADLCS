...
const { sendWelcomeEmail, sendTestEmail } = require('../lib/email')
...
router.get('/citizen-lookup', async (req, res) => {
  ...
  try {
    ...
    return res.status(404).json({ success: false, message: 'No citizen found with this Birth ID.' })
  } catch (err) {
...
router.post('/district-admins', requireRole('super_admin'), async (req, res) => {
  ...
  try {
    ...
    try {
      await sendWelcomeEmail({ to: email, fullName, role: 'district_admin', defaultPassword })
      emailSent = true
    } catch (err) {
      emailError = err.message
      console.error('[email/district-admin]', err.message)
    }
...
router.post('/village-officers', requireRole('district_admin'), async (req, res) => {
  ...
  try {
    ...
    let emailSent = false, emailError = null
    try {
      await sendWelcomeEmail({ to: email, fullName, role: 'village_officer', defaultPassword })
      emailSent = true
    } catch (err) {
      emailError = err.message
      console.error('[email/village-officer]', err.message)
    }
    return res.json({ success: true, data: { ...created, defaultPassword, emailSent, emailError } })
  } catch (err) {
    if (err.code === 'P2002') return res.status(409).json({ success: false, message: 'A record with this email or Birth ID already exists' })
...
router.post('/health-officers', requireRole('district_admin'), async (req, res) => {
  ...
  try {
    ...
    let emailSent = false, emailError = null
    try {
      await sendWelcomeEmail({ to: email, fullName, role: 'hospital_officer', defaultPassword })
      emailSent = true
    } catch (err) {
      emailError = err.message
      console.error('[email/hospital-officer]', err.message)
    }
    return res.json({ success: true, data: { ...created, defaultPassword, emailSent, emailError } })
  } catch (err) {
    if (err.code === 'P2002') return res.status(409).json({ success: false, message: 'A record with this email or Birth ID already exists' })
...
router.post('/super-admins', requireRole('super_admin'), async (req, res) => {
  ...
  try {
    ...
    let emailSent = false, emailError = null
    try {
      await sendWelcomeEmail({ to: email, fullName, role: 'super_admin', defaultPassword })
      emailSent = true
    } catch (err) {
      emailError = err.message
      console.error('[email/super-admin]', err.message)
    }
    return res.json({ success: true, data: { ...created, defaultPassword, emailSent, emailError } })
  } catch (err) {
