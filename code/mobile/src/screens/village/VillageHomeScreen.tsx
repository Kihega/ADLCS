...
import {
  ...
  ActivityIndicator, Image, ImageBackground, RefreshControl,
  Animated, Modal, TouchableWithoutFeedback, Share, Dimensions, InteractionManager,
} from 'react-native'
...
function OfficerIdCard({ visible, onClose, officer }: {
  visible:boolean; onClose:()=>void; officer:Record<string,any>
}) {
        ...
        <View style={ic.sheet}>
          ...
          <View style={ic.card}>
            <LinearGradient colors={['#02143c','#064e2e','#1eb53a']} style={ic.cardHeader}>
              <View style={ic.flag}>
                <View style={{flex:3,backgroundColor:TZ.green}}/><View style={{width:6,backgroundColor:TZ.yellow}}/>
                <View style={{width:5,backgroundColor:TZ.black}}/><View style={{width:6,backgroundColor:TZ.yellow}}/>
                <View style={{flex:3,backgroundColor:TZ.blue}}/>
              </View>
              <View style={ic.headerRow}>
                <Image source={require('../../../public/assets/court_of_arm.png')} style={{width:36,height:36}} resizeMode="contain"/>
                ...
                <Image source={require('../../../public/assets/longo_nbs.png')} style={{width:36,height:36}} resizeMode="contain"/>
              </View>
            </LinearGradient>
            <View style={ic.body}>
              <View style={ic.photoCol}>
                <View style={[ic.photo,{borderColor:G}]}><Text style={[ic.photoTxt,{color:G}]}>{initials}</Text></View>
                <View style={ic.activePill}>
                  <View style={{width:6,height:6,borderRadius:3,backgroundColor:G}}/><Text style={[ic.activeText,{color:G}]}>ACTIVE</Text>
                </View>
              </View>
              <View style={{flex:1}}>
                ...
                {[
                  ['Employee ID',  officer.employeeId ?? `NBS-VO-${Date.now().toString().slice(-6)}`],
                  ...
                  <View key={k} style={{flexDirection:'row',marginBottom:3}}>
                    ...
                    <Text style={ic.dv} numberOfLines={1}>{v}</Text>
                  </View>
                ))}
              </View>
            </View>
...
const ic = StyleSheet.create({
  ...
  headerRow:  {flexDirection:'row',alignItems:'center',paddingHorizontal:14,paddingTop:12,paddingBottom:8,gap:10},
  hTitle:     {fontSize:8,fontWeight:'900',color:'#fff',letterSpacing:1.5,textAlign:'center'},
  hSub:       {fontSize:7,color:'rgba(255,255,0.75)',textAlign:'center',marginTop:2},
  hRole:      {fontSize:10,fontWeight:'900',color:TZ.yellow,letterSpacing:2,textAlign:'center',marginTop:4},
  body:       {flexDirection:'row',backgroundColor:'#fff',padding:14,gap:12},
  photoCol:   {alignItems:'center',gap:8},
  photo:      {width:80,height:90,borderRadius:8,backgroundColor:'#e6f7ee',borderWidth:2,alignItems:'center',justifyContent:'center'},
  photoTxt:   {fontSize:28,fontWeight:'900'},
  activePill: {flexDirection:'row',alignItems:'center',gap:4,borderWidth:1,borderColor:'#1eb53a50',borderRadius:20,backgroundColor:'#1eb53a20',paddingHorizontal:8,paddingVertical:3},
  ...
  footer:     {backgroundColor:'#1e293b',paddingHorizontal:14,paddingVertical:10,alignItems:'center',gap:6},
  barcode:    {flexDirection:'row',alignItems:'center',height:32},
  footNote:   {fontSize:7,color:'rgba(255,255,0.50)',textAlign:'center'},
...
function Sidebar({ open, onClose, officer, onLogout, loggingOut, onShowProfile, navigation }: {
  ...
  const closeAndDo = (fn:()=>void) => {
    ...
    ]).start(()=>{ setMounted(false); onClose(); InteractionManager.runAfterInteractions(()=>fn()) })
  }
  ...
  const MENU = [
    { section:'VILLAGE', items:[
      { icon:<Building2 size={15} color={G}/>, label:'Village Details',
        onPress:()=>closeAndDo(()=>Alert.alert('Village Details',`${officer.villageName??'—'}\n${officer.wardName??'—'}`))},
    ]},
    { section:'ACCOUNT', items:[
      { icon:<IdCard size={15} color={G}/>, label:'View ID Card',
        onPress:()=>closeAndDo(()=>onShowProfile())},
      { icon:<User  size={15} color={G}/>, label:'View Profile',
        onPress:()=>closeAndDo(()=>onShowProfile())},
      { icon:<Lock  size={15} color={G}/>, label:'Change Password',
        onPress:()=>closeAndDo(()=>Alert.alert('Change Password','Use the NBS web admin panel.'))},
    ]},
    { section:'SESSION', items:[
      { icon:<LogOut size={15} color="#f87171"/>, label:loggingOut?'Signing out…':'Sign Out',
        danger:true, onPress:()=>closeAndDo(()=>onLogout())},
    ]},
  ]
    ...
    <Modal visible={mounted} transparent animationType="none" onRequestClose={onClose} statusBarTranslucent>
      ...
      <Animated.View style={[s.drawer,{backgroundColor:T.card,transform:[{translateX:tx}]}]}>
        ...
        <ScrollView style={{flex:1}} showsVerticalScrollIndicator={false}>
          {MENU.map(sec=>(
            <View key={sec.section} style={[s.drawerSection,{borderBottomColor:T.border}]}>
              ...
              {sec.items.map(item=>(
                <TouchableOpacity key={item.label} style={[s.drawerItem,{borderBottomColor:T.border}]}
                  onPress={item.onPress} activeOpacity={0.7}>
                  <View style={[s.drawerItemIcon,{backgroundColor:(item as any).danger?'rgba(239,68,0.12)':`${G}14`}]}>
                    {item.icon}
                  </View>
        ...
        <View style={[s.drawerFooter,{borderTopColor:T.border}]}>
          <Text style={[s.drawerFooterTxt,{color:T.textDim}]}>ADLCS · NBS Tanzania · © {new Date().getFullYear()}</Text>
        </View>
      </Animated.View>
    </Modal>
  )
...
export default function VillageHomeScreen({ navigation }: Props) {
  ...
  const [officer,     setOfficer]    = useState<Record<string,any>>({
    officerName:'Village Officer', villageName:'My Village', wardName:'—',
    totalCitizens:0, employeeId:'',
  })
  ...
  return (
    <SafeAreaView style={{flex:1,backgroundColor:T.bg}} edges={['top']}>
      ...
      <ImageBackground source={require('../../../public/assets/flag.jpg')} style={{overflow:'hidden'}} blurRadius={2} resizeMode="cover">
        <LinearGradient
          colors={isDark?['rgba(2,20,60,0.70)','rgba(5,40,20,0.70)']:['rgba(0,50,20,0.65)','rgba(0,80,30,0.60)']}
        ...
        <View style={{flexDirection:'row',alignItems:'center',paddingHorizontal:12,paddingBottom:12,paddingTop:2,gap:8}}>
          ...
          <View style={{flex:1}}>
            ...
            <Text style={{fontSize:9,color:'rgba(255,255,0.55)',marginTop:2}} numberOfLines={1}>
      ...
      {/* ── SCROLL BODY ── */}
      <ScrollView showsVerticalScrollIndicator={false} style={{backgroundColor:T.bg}}
        contentContainerStyle={{paddingBottom:36}}
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={()=>{setRefreshing(true);loadData()}} tintColor={G}/>}>
        ...
        <View style={[s.welcomeCard,{backgroundColor:T.card,borderColor:T.border}]}>
          ...
          <View style={{alignItems:'flex-end',gap:6}}>
            <View style={[s.zoneBadge,{borderColor:inZone?`${G}60`:'rgba(239,68,0.38)',backgroundColor:inZone?`${G}18`:'rgba(239,68,0.12)'}]}>
              {inZone ? <MapPin size={9} color={G}/> : <AlertTriangle size={9} color="#f87171"/>}
              <Text style={[s.zoneTxt,{color:inZone?G:'#f87171'}]}>
        ...
        {/* Village info card */}
        <View style={s.sectionHead}><Text style={[s.sectionTitle,{color:T.text}]}>Village Information</Text></View>
        <View style={[s.facilityCard,{backgroundColor:T.card,borderColor:T.border}]}>
          <LinearGradient colors={[`${G}22`,`${G}08`]} start={{x:0,y:0}} end={{x:1,y:0}}
            style={{flexDirection:'row',alignItems:'center',gap:12,padding:16}}>
            <View style={[s.facilityIcon,{backgroundColor:`${G}25`}]}><Building2 size={22} color={G}/></View>
            <View style={{flex:1}}>
              <Text style={{fontSize:13,fontWeight:'800',color:T.text,marginBottom:2}}>{officer.villageName}</Text>
              <Text style={{fontSize:11,color:T.textSub,marginBottom:2}}>Ward: {officer.wardName}</Text>
              <View style={{flexDirection:'row',alignItems:'center',gap:4}}>
                <Shield size={10} color={G}/>
                <Text style={{fontSize:10,color:T.textDim}}>NBS Authorised Village Registration Unit</Text>
              </View>
            </View>
          </LinearGradient>
        ...
        <View style={[s.reportCard,{backgroundColor:T.card,borderColor:T.border}]}>
          {[
            {label:'Total citizens registered', value:officer.totalCitizens??0, color:TZ.blue},
          ...
          <View style={[s.downloadSection,{borderTopColor:T.border}]}>
            <Text style={{fontSize:11,fontWeight:'700',color:T.textSub,marginBottom:10}}>Download Report</Text>
            <View style={{flexDirection:'row',gap:8}}>
              {(['Daily','Weekly','Monthly','Annual'] as const).map(p=>(
                <TouchableOpacity key={p}
                  style={{flex:1,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:3,borderWidth:1,borderRadius:8,paddingVertical:6,backgroundColor:`${G}18`,borderColor:`${G}40`}}
                  onPress={()=>downloadReport(p)}>
                  <Download size={10} color={G}/><Text style={{fontSize:9,fontWeight:'700',color:G}}>{p}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </View>
...
const s = StyleSheet.create({
  ...
  drawerAvatar:      {width:52,height:52,borderRadius:26,alignItems:'center',justifyContent:'center',borderWidth:2},
  drawerAvatarTxt:   {fontSize:18,fontWeight:'900',color:'#003087'},
  drawerName:        {fontSize:14,fontWeight:'800',color:'#fff',marginBottom:3},
  drawerSub:         {fontSize:11,color:'rgba(255,255,0.65)'},
  ...
  actionCard:        {flex:1,borderRadius:12,borderWidth:1,padding:10,alignItems:'center',gap:5},
