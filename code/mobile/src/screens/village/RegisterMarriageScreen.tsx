...
function genCertNo() { return `TZ-MAR-${Date.now().toString().slice(-8)} M` }
...
export default function RegisterMarriageScreen({ navigation }: Props) {
  ...
  const handleSubmit = async () => {
    if (!marriageDate) { Alert.alert('Required','Select the marriage date.'); return }
    if (!witness1.trim()) { Alert.alert('Required','Enter at least one witness name.'); return }
    setSubmitting(true)
    try {
      const cert = genCertNo(); setCertNo(cert)
      const token = await AsyncStorage.getItem('adlcs_access_token')
      if (isOnline() && token) {
        try {
          await apiPost('/village/marriage', {
            husbandNid, husbandName: husbandData ? `${husbandData.firstName} ${husbandData.surname}` : husbandNid,
            wifeNid,    wifeName:    wifeData    ? `${wifeData.firstName} ${wifeData.surname}`       : wifeNid,
            marriageDate, marriageType, witness1:witness1.trim(), witness2:witness2.trim(),
            bridePrice:bridePrice.trim(), certNo:cert,
          })
        } catch {}
      }
      setStep(4)
  ...
  const NidSearch = ({ label, nid, setNid, data, loading, party, color }: any) => (
    <View>
      <Text style={{fontSize:12,fontWeight:'600',color:T.textSub,marginBottom:6}}>{label} National ID *</Text>
      <View style={{flexDirection:'row',gap:8,marginBottom:8}}>
        <TextInput style={{flex:1,borderWidth:1,borderRadius:10,paddingHorizontal:14,paddingVertical:12,fontSize:14,backgroundColor:T.card2,borderColor:T.border,color:T.text}}
          value={nid} onChangeText={setNid} placeholder="YYYYMMDD-LLLLL-SSSSS-CC"
          placeholderTextColor={T.textDim} returnKeyType="search" blurOnSubmit={false}/>
        <TouchableOpacity style={{backgroundColor:color,borderRadius:10,paddingHorizontal:14,alignItems:'center',justifyContent:'center',opacity:nid.trim().length>=5?1:0.4}}
          onPress={()=>searchParty(nid,party)} disabled={nid.trim().length<5||loading}>
          {loading ? <ActivityIndicator color="#fff" size="small"/> : <Search size={16} color="#fff"/>}
        </TouchableOpacity>
      </View>
      {data && (
        <View style={{borderWidth:1,borderRadius:10,borderColor:`${color}40`,backgroundColor:`${color}10`,padding:12,flexDirection:'row',alignItems:'center',gap:10}}>
          <User size={14} color={color}/>
          <View style={{flex:1}}>
            <Text style={{fontSize:13,fontWeight:'700',color:T.text}}>{data.firstName} {data.middleName??''} {data.surname}</Text>
            <Text style={{fontSize:11,color:T.textSub}}>{data.nationalId} · {data.gender?.toUpperCase()}</Text>
          </View>
  ...
  return (
    <SafeAreaView style={{flex:1,backgroundColor:T.bg}} edges={['top']}>
      <KeyboardAvoidingView style={{flex:1}} behavior={Platform.OS==='ios'?'padding':undefined}>
        <ScreenHeader title="Register Marriage" sub="Civil Registration Act (Cap 29)"
          icon={<Heart size={18} color="#e11d48"/>} iconBg="#e11d4818"
          onBack={()=>step>1&&step<4?setStep(s=>(s-1) as any):navigation.goBack()}/>
        ...
        <View style={{flexDirection:'row',backgroundColor:T.card,borderBottomWidth:1,borderBottomColor:T.border}}>
          {['Husband','Wife','Details','Certificate'].map((lbl,i)=>(
            <View key={lbl} style={{flex:1,alignItems:'center',paddingVertical:10}}>
              <View style={{width:20,height:20,borderRadius:10,alignItems:'center',justifyContent:'center',marginBottom:3,
        ...
        <ScrollView contentContainerStyle={{padding:20,paddingBottom:40}} keyboardShouldPersistTaps="handled">
          {step===1 && (
            <View style={{gap:16}}>
              <Text style={{fontSize:16,fontWeight:'800',color:T.text}}>Husband Identification</Text>
              <NidSearch label="Husband" nid={husbandNid} setNid={setHusbandNid}
                data={husbandData} loading={husbandLoading} party="husband" color="#0891b2"/>
              {!husbandData && <SField label="Or enter name manually" value={husbandNid} onChange={setHusbandNid}
                placeholder="Full name if NID unavailable" bg={T.card2} bc={T.border} tc={T.text} dc={T.textDim}/>}
              <TouchableOpacity style={{backgroundColor:'#e11d48',borderRadius:12,paddingVertical:14,alignItems:'center',opacity:husbandNid.trim()?1:0.4}}
                onPress={()=>setStep(2)} disabled={!husbandNid.trim()}>
          ...
          {step===2 && (
            <View style={{gap:16}}>
              <Text style={{fontSize:16,fontWeight:'800',color:T.text}}>Wife Identification</Text>
              <NidSearch label="Wife" nid={wifeNid} setNid={setWifeNid}
                data={wifeData} loading={wifeLoading} party="wife" color="#7c3aed"/>
              {!wifeData && <SField label="Or enter name manually" value={wifeNid} onChange={setWifeNid}
                placeholder="Full name if NID unavailable" bg={T.card2} bc={T.border} tc={T.text} dc={T.textDim}/>}
              <TouchableOpacity style={{backgroundColor:'#e11d48',borderRadius:12,paddingVertical:14,alignItems:'center',opacity:wifeNid.trim()?1:0.4}}
                onPress={()=>setStep(3)} disabled={!wifeNid.trim()}>
                <Text style={{color:'#fff',fontWeight:'800',fontSize:14}}>Continue →</Text>
              </TouchableOpacity>
            </View>
          )}
          ...
          {step===3 && (
            <View style={{gap:14}}>
              <Text style={{fontSize:16,fontWeight:'800',color:T.text}}>Marriage Details</Text>
              <View>
                <Text style={{fontSize:12,fontWeight:'600',color:T.textSub,marginBottom:6}}>Date of Marriage *</Text>
                <TouchableOpacity style={{borderWidth:1,borderRadius:10,paddingHorizontal:14,paddingVertical:12,backgroundColor:T.card2,borderColor:T.border,flexDirection:'row',alignItems:'center',justifyContent:'space-between'}}
                  ...
                  <Text style={{color:marriageDate?T.text:T.textDim,fontSize:14}}>{marriageDate||'Select date'}</Text>
                  <Calendar size={16} color={T.textDim}/>
                </TouchableOpacity>
              </View>
              <View>
                <Text style={{fontSize:12,fontWeight:'600',color:T.textSub,marginBottom:8}}>Type of Marriage *</Text>
                <View style={{gap:8}}>
                  {MTYPES.map(mt=>(
                    <TouchableOpacity key={mt.val} onPress={()=>setMarriageType(mt.val)}
                      style={{flexDirection:'row',alignItems:'center',gap:12,borderWidth:1.5,borderRadius:12,padding:14,
                              borderColor:marriageType===mt.val?mt.color:T.border,backgroundColor:marriageType===mt.val?`${mt.color}15`:T.card2}}>
                      <View style={{width:18,height:18,borderRadius:9,borderWidth:2,borderColor:mt.color,alignItems:'center',justifyContent:'center'}}>
                        {marriageType===mt.val && <View style={{width:9,height:9,borderRadius:4.5,backgroundColor:mt.color}}/>}
                      </View>
                      <Text style={{fontSize:14,fontWeight:'600',color:marriageType===mt.val?mt.color:T.text}}>{mt.label}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
              </View>
              <SField label="Witness 1 Full Name" value={witness1} onChange={setWitness1}
                placeholder="Name of first witness" required bg={T.card2} bc={T.border} tc={T.text} dc={T.textDim}/>
              <SField label="Witness 2 Full Name" value={witness2} onChange={setWitness2}
                placeholder="Name of second witness (optional)" bg={T.card2} bc={T.border} tc={T.text} dc={T.textDim}/>
              <SField label="Bride Price / Mahari" value={bridePrice} onChange={setBridePrice}
                placeholder="e.g. 5 cattle, TZS 500,000 (optional)" bg={T.card2} bc={T.border} tc={T.text} dc={T.textDim}/>
              <TouchableOpacity style={{backgroundColor:'#e11d48',borderRadius:12,paddingVertical:14,alignItems:'center',opacity:submitting?0.7:1}}
                onPress={handleSubmit} disabled={submitting}>
                {submitting ? <ActivityIndicator color="#fff"/> : <Text style={{color:'#fff',fontWeight:'800',fontSize:14}}>Register Marriage</Text>}
              </TouchableOpacity>
            </View>
          )}
          ...
          {step===4 && (
            <View style={{alignItems:'center',gap:14}}>
              <View style={{width:80,height:80,borderRadius:40,backgroundColor:'#e11d4818',alignItems:'center',justifyContent:'center'}}>
                <Heart size={40} color="#e11d48"/>
              </View>
              <Text style={{fontSize:22,fontWeight:'900',color:T.text}}>Marriage Registered!</Text>
              ...
              <View style={{width:'100%',borderRadius:12,borderWidth:1,borderColor:'#e11d4850',backgroundColor:'#e11d4810',padding:14}}>
                <Text style={{fontSize:11,color:T.textDim,marginBottom:4}}>MARRIAGE CERTIFICATE NUMBER</Text>
                <View style={{flexDirection:'row',alignItems:'center'}}>
                  <Text style={{fontSize:16,fontWeight:'900',color:'#e11d48',flex:1,letterSpacing:1}}>{certNo}</Text>
                  <TouchableOpacity onPress={()=>copy(certNo,'Certificate number')} style={{padding:4}}>
                    <Copy size={15} color="#e11d48"/>
                  </TouchableOpacity>
                </View>
              </View>
              <View style={{flexDirection:'row',gap:8,width:'100%',backgroundColor:'#0891b212',borderRadius:10,padding:12}}>
                <Shield size={13} color="#0891b2"/>
                <Text style={{fontSize:10,color:T.textSub,flex:1}}>Certificate will be generated and available in the Certificates screen after sync.</Text>
              </View>
              <TouchableOpacity style={{backgroundColor:'#e11d48',borderRadius:12,paddingVertical:14,alignItems:'center',width:'100%'}}
        ...
        <CalPicker visible={showCal} title="Date of Marriage" maxDate={new Date()}
          onSelect={d=>{setMarriageDate(d);setShowCal(false)}} onClose={()=>setShowCal(false)}/>
