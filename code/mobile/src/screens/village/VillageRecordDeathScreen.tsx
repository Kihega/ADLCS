      ...
      if (token && isOnline()) {
        try {
          ...
          if (j.success) {
            ...
            facilityName = j.data.villageName ?? j.data.facilityName
          }
        } catch {}
      }
      await saveDeath({
        ...
        deceasedName:
          (citizen?.fullName ?? citizen?.firstName)
        ...
        facilityName: facilityName || 'Village',
        officerName,
        rawJson: JSON.stringify({ nationalId: citizen?.nationalId, causeOfDeath: cause.trim() }),
      })
      if (token && isOnline()) {
        try {
          await apiPost('/village/death', {
            deceasedName: citizen?.fullName ?? lookupId.trim(),
            nationalId: citizen?.nationalId ?? lookupId.trim(),
            causeOfDeath: cause.trim(),
            dateOfDeath: dod.trim(),
            category: category,
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: T.bg }} edges={['top']}>
      ...
      >
        <ScreenHeader
          title="Record Death"
          sub="Outside Health Facility"
          icon={<Cross size={18} color="#dc2626" />}
          iconBg="#dc262618"
          onBack={() =>
            step > 1 && step < 3 ? setStep((s) => (s - 1) as any) : navigation.goBack()
          }
        />
        ...
        >
          ...
          >
            ...
            <Text style={{ fontSize: 12, color: T.textSub, flex: 1, lineHeight: 18 }}>
              Record deaths that occurred{' '}
              <Text style={{ fontWeight: '800', color: '#f97316' }}>OUTSIDE health facilities</Text>
              . Deaths at hospitals or dispensaries are recorded by the Hospital Officer.
            </Text>
          </View>
              ...
              <View style={{ flexDirection: 'row', gap: 8 }}>
                ...
                <TouchableOpacity
                  style={{
                    ...
                    alignItems: 'center',
                    justifyContent: 'center',
                    opacity: lookupId.trim().length >= 3 ? 1 : 0.4,
                  }}
          ...
          {step === 2 && (
            <View style={{ gap: 0 }}>
              ...
              <SField
                ...
                value={cause}
                onChange={setCause}
                placeholder="e.g. Malaria, Accident, Cardiac arrest"
                required
                multiline
                bg={T.card2}
                bc={T.border}
                  ...
                  {LOC_TYPES.map((l) => (
                    ...
                    >
                      <Text
                        style={{
                          fontSize: 11,
                          color: locationType === l.val ? '#dc2626' : T.textSub,
                        }}
                      >
                        {l.label}
                      </Text>
              ...
              <View style={{ marginBottom: 14 }}>
                ...
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
                  {CATEGORIES.map((c) => (
                    ...
                    >
                      <Text
                        style={{ fontSize: 11, color: category === c.val ? '#dc2626' : T.textSub }}
                      >
                        {c.label}
                      </Text>
                    </TouchableOpacity>
          ...
          {step === 3 && (
            <View style={{ alignItems: 'center', gap: 14 }}>
              ...
              <Text style={{ fontSize: 12, color: T.textSub, textAlign: 'center' }}>
                Saved locally · will sync to Central Database on reconnection
              </Text>
              <View
                style={{
                  width: '100%',
