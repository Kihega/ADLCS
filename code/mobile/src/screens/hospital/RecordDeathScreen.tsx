      ...
      <View style={{ flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(0,0,0,0.65)' }}>
        ...
        >
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 14 }}>
            ...
            <Text style={{ flex: 1, fontSize: 15, fontWeight: '800', color: '#f8fafc' }}>
              {title}
            </Text>
          ...
          <View style={{ flexDirection: 'row', gap: 6, height: 200 }}>
            ...
            <View style={{ flex: 1 }}>
              <Text
                style={{ fontSize: 10, color: '#4b6080', textAlign: 'center', marginBottom: 4 }}
              >
                Day
              </Text>
                ...
                {Array.from({ length: daysIn }, (_, i) => i + 1).map((d) => (
                  <TouchableOpacity
                    ...
                    style={{
                      ...
                      borderRadius: 8,
                      marginVertical: 2,
                      backgroundColor: d === day ? '#0891b230' : 'transparent',
                    }}
                  >
                    <Text
                      style={{
                        fontSize: 13,
                        color: d === day ? '#22d3ee' : '#94a3b8',
  ...
  return (
    <View style={{ marginBottom: 14 }}>
      ...
      <TextInput
        ...
        style={[
          sf.input,
          { backgroundColor: bg, borderColor: bc, color: tc },
          multiline && sf.multi,
        ]}
        value={value}
...
export default function RecordDeathScreen({ navigation }: Props) {
  ...
  const lookupCitizen = useCallback(async () => {
    if (!lookupId.trim()) {
      Alert.alert('Error', 'Enter a National ID or name.')
      return
    }
    setLookupLoad(true)
    try {
    ...
    Alert.alert('Not Found in DB', 'No record found online. Proceed with manual name entry?', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Manual Entry',
        onPress: () => {
          setCitizen({ fullName: lookupId.trim(), nationalId: lookupId.trim() })
        },
      },
    ])
    setLookupLoad(false)
  ...
  const lookupParent = useCallback(async (nid: string, role: 'father' | 'mother') => {
    ...
    try {
      ...
      if (token) {
        ...
        if (json.success && json.data) {
          const d = json.data
          if (isFather && d.gender?.toUpperCase() !== 'MALE') {
            setErr('This NID belongs to a female citizen.')
            setLoad(false)
            return
          }
          if (!isFather && d.gender?.toUpperCase() !== 'FEMALE') {
            setErr('This NID belongs to a male citizen.')
            setLoad(false)
            return
          }
          setData(d)
  ...
  const handleSubmit = useCallback(async () => {
    if (!cause.trim()) {
      Alert.alert('Required', 'Enter cause of death.')
      return
    }
    if (!dod.trim()) {
      Alert.alert('Required', 'Select date of death.')
      return
    }
    if (!informant.trim()) {
      Alert.alert('Required', 'Enter informant name.')
      return
    }
    setSubmitting(true)
    ...
    try {
      ...
      if (token) {
        try {
          ...
          if (j.success) {
            officerName = j.data.officerName
            facilityName = j.data.facilityName
          }
        } catch {}
      }
      ...
      if (deathType === 'infant') {
        ...
        deceasedName = `INFANT OF ${
          fatherData
            ? `${fatherData.firstName ?? ''} ${fatherData.surname ?? ''}`.trim()
            : 'UNKNOWN FATHER'
        } & ${
          motherData
            ? `${motherData.firstName ?? ''} ${motherData.surname ?? ''}`.trim()
            : 'UNKNOWN MOTHER'
        }`.toUpperCase()
      } else {
  ...
  const step2Ready = deathType === 'adult' ? !!citizen : !!(fatherData && motherData)
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: T.bg }} edges={['top']}>
      ...
      >
        ...
        <View style={[s.header, { backgroundColor: T.card, borderBottomColor: T.border }]}>
          ...
          <View style={[s.backBtn, { backgroundColor: '#dc262618' }]}>
            ...
            <View key={lbl} style={{ alignItems: 'center', flex: 1 }}>
              <View
                style={[
                  ...
                  {
                    backgroundColor:
                      step > i + 1 ? T.success : step === i + 1 ? '#dc2626' : T.border,
                  },
                ]}
              >
                {step > i + 1 ? (
              ...
              <Text style={[s.stepLabel, { color: step >= i + 1 ? T.text : T.textDim }]}>
                {lbl}
              </Text>
            </View>
          ))}
                ...
                <View style={{ flex: 1, gap: 3 }}>
                  <Text
                    style={{
                      fontSize: 16,
                      fontWeight: '800',
                      color: deathType === 'infant' ? '#dc2626' : T.text,
                    }}
                  >
                    INFANT
                  </Text>
                  <Text style={{ fontSize: 12, color: T.textSub, lineHeight: 17 }}>
                    Death before or shortly after birth. Father + Mother NIDs required. No citizen
                    NIN exists yet.
                  </Text>
                </View>
                ...
                <View style={{ flex: 1, gap: 3 }}>
                  <Text
                    style={{
                      fontSize: 16,
                      fontWeight: '800',
                      color: deathType === 'adult' ? '#dc2626' : T.text,
                    }}
                  >
                    ADULT
                  </Text>
          ...
          {step === 2 && deathType === 'adult' && (
            <View style={{ gap: 14 }}>
              ...
              <Text style={{ fontSize: 12, color: T.textSub, lineHeight: 18 }}>
                Enter the National ID or full name. Vital status will be updated to DECEASED upon
                submission.
              </Text>
              <View>
                <Text
                ...
                <View style={{ flexDirection: 'row', gap: 8 }}>
                  <TextInput
                    style={[
                      sf.input,
                      { flex: 1, backgroundColor: T.card2, borderColor: T.border, color: T.text },
                    ]}
                    value={lookupId}
                  ...
                  <TouchableOpacity
                    ...
                    disabled={lookupId.trim().length < 3 || lookupLoad}
                  >
                    {lookupLoad ? (
                  ...
                  >
                    ...
                    <View style={{ flexDirection: 'row', gap: 8 }}>
                      ...
                      <TouchableOpacity
                        style={[
                          s.searchBtn,
                          { backgroundColor: accent, opacity: isNINComplete(nid) ? 1 : 0.4 },
                        ]}
                    ...
                    {!!error && (
                      <Text style={{ fontSize: 11, color: '#f87171', lineHeight: 17 }}>
                        ⚠ {error}
                      </Text>
                    )}
                    {data && (
                      ...
                      >
                        ...
                        <View style={{ flex: 1, marginLeft: 10 }}>
                          ...
                          <Text style={{ fontSize: 10, color: T.textSub, marginTop: 2 }}>
                            {data.nationalId}
                          </Text>
                        </View>
          ...
          {step === 3 && (
            <View>
              ...
              <View
                style={[
                  s.citizenCard,
                  { backgroundColor: T.card, borderColor: T.border, marginBottom: 16, gap: 4 },
                ]}
              >
                ...
                <View style={{ flex: 1, marginLeft: 10 }}>
                  ...
                  {deathType === 'infant' && (
                    <>
                      ...
                      <Text style={{ fontSize: 11, color: T.textSub, marginTop: 2 }}>
                        Father: {fatherData?.firstName} {fatherData?.surname} · Mother:{' '}
                        {motherData?.firstName} {motherData?.surname}
                      </Text>
                    </>
                  )}
                </View>
              ...
              <View style={{ marginBottom: 14 }}>
                ...
                <TouchableOpacity
                  ...
                  activeOpacity={0.8}
                >
                  <Text style={{ color: dod ? T.text : T.textDim, fontSize: 14 }}>
                    {dod || 'Select date of death'}
                  </Text>
                  ...
                  {LOC_TYPES.map((l) => (
                    <TouchableOpacity
                      ...
                      style={[
                        s.chip,
                        {
                          borderColor: locType === l.val ? '#dc2626' : T.border,
                          backgroundColor: locType === l.val ? '#dc262618' : T.card,
                        },
                      ]}
                    >
                      <Text
                        style={{ fontSize: 11, color: locType === l.val ? '#dc2626' : T.textSub }}
                      >
                        {l.label}
                      </Text>
                    </TouchableOpacity>
                    ...
                    {ADULT_CATEGORIES.map((c) => (
                      ...
                      >
                        <Text
                          style={{
                            fontSize: 11,
                            color: category === c.val ? '#dc2626' : T.textSub,
                          }}
                        >
                          {c.label}
                        </Text>
          ...
          {step === 4 && (
            <View style={{ alignItems: 'center', paddingTop: 24, gap: 14 }}>
              ...
              <Text style={{ fontSize: 13, color: T.textSub }}>
                {deathType === 'infant' ? 'Infant death record saved' : 'Citizen status → DECEASED'}{' '}
              ...
              <View style={{ width: '100%', gap: 6 }}>
                ...
                >
                  ...
                  <TouchableOpacity
                    onPress={() => copy(savedCertNo, 'Certificate number')}
                    style={{ padding: 4 }}
                  >
              ...
              >
                ...
                <Text style={{ fontSize: 10, color: T.textSub, flex: 1 }}>
                  Record stored locally · will sync to Central Database on next connection · QR
                  signed by Govt PKI
                </Text>
              </View>
              ...
              <TouchableOpacity
                style={{
                  ...
                  justifyContent: 'center',
                  gap: 8,
                  backgroundColor: downloading ? T.card2 : '#dc2626',
                  borderRadius: 12,
                  paddingVertical: 14,
                  width: '100%',
              ...
              >
                <Text style={{ fontSize: 14, fontWeight: '700', color: T.textSub }}>
                  Back to Dashboard
                </Text>
              </TouchableOpacity>
            </View>
