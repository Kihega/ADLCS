      ...
      <View style={[s.card, { backgroundColor:T.card, borderColor:T.border }]}>
        <View style={{ flexDirection:'row', alignItems:'flex-start', gap:12 }}>
          <View style={[s.icon, { backgroundColor:`${color}18` }]}>
            {item.type==='birth' ? <Baby size={16} color={color} /> : <Cross size={16} color={color} />}
          </View>
          <View style={{ flex:1 }}>
        ...
        <View style={{ marginTop:10, gap:5 }}>
          {item.reasons.map(r=>(
            <View key={r} style={[s.reasonRow, { backgroundColor:'#f9731610', borderColor:'#f9731630' }]}>
              ...
              <Text style={{ fontSize:11, color:'#f97316', flex:1 }}>{REASON_LABELS[r]??r}</Text>
            </View>
          ))}
        </View>
        <View style={{ flexDirection:'row', gap:8, marginTop:10 }}>
          <TouchableOpacity style={[s.actionBtn, { backgroundColor:'#0891b218', borderColor:'#0891b260' }]}
            onPress={()=>navigation.navigate('IssueCertificate')}>
            <FileText size={12} color="#0891b2" />
            <Text style={{ fontSize:11, color:'#0891b2', fontWeight:'700' }}>Issue Certificate</Text>
          </TouchableOpacity>
        </View>
      </View>
    )
  ...
  return (
    <SafeAreaView style={{ flex:1, backgroundColor:T.bg }} edges={['top']}>
      <View style={[s.header, { backgroundColor:T.card, borderBottomColor:T.border }]}>
        ...
        <View style={[s.backBtn, { backgroundColor:'#f9731618' }]}><Clock size={18} color="#f97316" /></View>
      </View>
