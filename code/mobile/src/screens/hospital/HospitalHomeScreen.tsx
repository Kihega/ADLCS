/**
 * HospitalHomeScreen.tsx  v6.0  PRODUCTION
 *
 * FIXES:
 *  • Sidebar: proper mount/unmount cycle — no app stack freeze
 *  • Sidebar navigation happens AFTER close animation completes
 *  • Officer ID Card modal — government employee ID design, fetches from DB
 *  • ConnQuality from NetInfo state (not ping), instant response
  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      <View style={ic.overlay}>
        <View style={ic.sheet}>
          <View style={[ic.sheetHandle, { backgroundColor: T.border }]} />
          ...
          <View style={ic.card}>
            ...
            <View style={ic.cardFooter}>
              ...
              <Text style={ic.footerNote}>
                If found, please return to the nearest NBS office. · NBS Tanzania © {issueYear}
              </Text>
            </View>
          </View>
...
function Sidebar({ open, onClose, officer, onLogout, loggingOut, onShowProfile, navigation }: {
  ...
  // Navigate AFTER drawer finishes closing — prevents navigation stack freeze
...
export default function HospitalHomeScreen({ navigation }: Props) {
  ...
  const downloadReport = async (period: string, type: string) => {
    const lines = [
      ...
      `──────────────────────────────`,
      `Births today:  ${stats.todayBirths}`,
