...
export default function SyncDataScreen({ navigation }: Props) {
  ...
  const doSync = async () => {
    ...
    await loadStatus()
    if (result.offline)
      Alert.alert('Offline', 'Device is offline. Records queued for next connection.')
    else if (result.synced > 0)
      Alert.alert('Sync Complete', `${result.synced} records pushed to Central Database.`)
    else if (result.failed > 0)
      Alert.alert('Partial Sync', `${result.failed} records failed. Will retry automatically.`)
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: T.bg }} edges={['top']}>
      <View style={[ss.header, { backgroundColor: T.card, borderBottomColor: T.border }]}>
        ...
        <View style={[ss.backBtn, { backgroundColor: '#0e749018' }]}>
        ...
        >
          ...
          <View style={[ss.banner, { backgroundColor: `${color}15`, borderColor: `${color}40` }]}>
            ...
            <View style={{ flex: 1, marginLeft: 12 }}>
              <Text style={[ss.bannerTitle, { color }]}>
                {quality === 'Good'
              ...
              <Text style={[ss.bannerSub, { color: T.textSub }]}>
                {quality !== 'Offline'
                  ? 'Real-time sync with Central Database is active'
                  : 'All records safely queued locally. Will sync automatically on reconnection.'}
              </Text>
            </View>
          </View>
          {/* Stats */}
          <View style={[ss.statsCard, { backgroundColor: T.card, borderColor: T.border }]}>
            <Row
              icon={<Baby size={14} color={TZ.green} />}
              label="Unsynced births"
              value={String(syncData.unsyncedBirths)}
              vc={syncData.unsyncedBirths ? '#f97316' : T.success}
            />
            <Row
              icon={<Cross size={14} color="#dc2626" />}
              label="Unsynced deaths"
              value={String(syncData.unsyncedDeaths)}
              vc={syncData.unsyncedDeaths ? '#f97316' : T.success}
            />
            <Row
              icon={<Clock size={14} color={T.primary} />}
              label="Last sync"
              value={
                syncData.lastSyncAt
                  ? new Date(syncData.lastSyncAt).toLocaleString('en-TZ', {
                      day: 'numeric',
                      month: 'short',
                      hour: '2-digit',
                      minute: '2-digit',
            ...
            <View style={[ss.row, { borderBottomWidth: 0 }]}>
              <View style={[ss.rowIcon, { backgroundColor: T.card2 }]}>
                <Shield size={14} color={quality !== 'Offline' ? T.success : T.danger} />
              </View>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
                <View
                  style={{
                    ...
                    height: 7,
                    borderRadius: 3.5,
                    backgroundColor: quality !== 'Offline' ? T.success : T.danger,
                  }}
                />
                <Text style={[ss.rowVal, { color: quality !== 'Offline' ? T.success : T.danger }]}>
                  {quality !== 'Offline' ? 'Reachable' : 'Unreachable'}
                </Text>
              </View>
            </View>
          </View>
                ...
                >
                  <View
                    style={{
                      ...
                      height: 8,
                      borderRadius: 4,
                      backgroundColor: CONN_COLORS[q],
                    }}
                  />
                  <Text
                    style={{
                      fontSize: 12,
                      fontWeight: q === quality ? '800' : '500',
                      color: q === quality ? CONN_COLORS[q] : T.textDim,
                    }}
                  >
                    {q}
                  </Text>
          ...
          >
            ...
            <Text style={[ss.syncBtnTxt, { color: syncing ? '#0e7490' : '#fff' }]}>
          ...
          <View style={[ss.secNote, { backgroundColor: T.card2, borderColor: T.border }]}>
            ...
            <Text style={{ flex: 1, fontSize: 10, color: T.textDim, lineHeight: 16 }}>
              All data transmitted over TLS 1.3. Offline records AES-encrypted on-device. Sync
              events timestamped in audit log.
            </Text>
          </View>
        </ScrollView>
      )}
